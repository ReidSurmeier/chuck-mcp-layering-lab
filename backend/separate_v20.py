...
def build_zip_response(image_bytes, plates=4, dust=50,
    ...
    with zipfile.ZipFile(zip_buf, 'w', zipfile.ZIP_DEFLATED) as zf:
        ...
        from color_names import plate_filename_stem

        for plate_info in manifest["plates"]:
            name = plate_info["name"]
            plate_data = result["plates"][name]
            fname = plate_filename_stem(plate_info["color"], plate_info["rank"])
            # Remember the on-disk filename so the manifest mirrors the ZIP.
            plate_info["filename"] = fname

            png_buf = io.BytesIO()
            plate_data["image"].save(png_buf, format="PNG")
            zf.writestr(zipfile.ZipInfo(f"png/{fname}.png"), png_buf.getvalue(), compress_type=zipfile.ZIP_STORED)

            svg_content = mask_to_svg_string(plate_data["mask"], w, h)
            zf.writestr(zipfile.ZipInfo(f"svg/{fname}.svg"), svg_content.encode(), compress_type=zipfile.ZIP_DEFLATED)
