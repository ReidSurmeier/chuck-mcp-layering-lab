...
async def job_status(job_id: str):
    ...
    if status == JobStatus.DONE:
        ...
        # Wrap result in status envelope so frontend can distinguish
        # done from pending via pollData.status === "done"
        result_str = result.decode("utf-8") if isinstance(result, bytes) else result
        return Response(
            content=json.dumps({"status": "done", "plates": json.loads(result_str)}),
