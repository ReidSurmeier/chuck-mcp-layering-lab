...
def separate(input_path_or_array, output_dir=None, n_plates=4, dust_threshold=150,
    ...
    # Instead of smoothing individual plate masks (which creates gaps),
    # smooth the label map itself: for each plate, create a soft distance field,
    # then reassign each pixel to whichever plate has the strongest claim.
    # This rounds jagged boundaries while keeping every pixel assigned.
    # Create a probability-like map for each plate using distance from boundary
    plate_scores = np.zeros((n_plates, h, w), dtype=np.float32)
    for i in range(n_plates):
        mask_float = (pixel_labels == i).astype(np.float32)
        # Gaussian blur creates soft boundaries — pixels near edges get partial scores
        plate_scores[i] = cv2.GaussianBlur(mask_float, (9, 9), sigmaX=2.5)

    # Reassign every pixel to the plate with highest score
    # Background pixels (where all scores are ~0) keep their original assignment
