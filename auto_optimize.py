...
def _validate_job_id(job_id: str) -> None:
    """Validate job_id is exactly 12 hex chars to prevent path traversal."""
    if not re.fullmatch(r"[a-f0-9]{12}", job_id):
        raise ValueError(f"Invalid job_id: {job_id}")


def _cleanup_stale_jobs() -> None:
    """Remove optimization job directories older than TTL."""
    import time
    if not os.path.isdir(OPTIMIZE_DIR):
        return
    for d in _glob.glob(os.path.join(OPTIMIZE_DIR, "*")):
        if os.path.isdir(d) and time.time() - os.path.getmtime(d) > _JOB_TTL_SECONDS:
...
def trigger_optimization(image_bytes: bytes, initial_plates: int = 8) -> dict:
    ...
    existing = [d for d in _glob.glob(os.path.join(OPTIMIZE_DIR, "*")) if os.path.isdir(d)]
    if len(existing) >= _MAX_CONCURRENT_JOBS:
        raise RuntimeError("Too many concurrent optimization jobs. Try again later.")

    # Generate job ID from image hash
    job_id = hashlib.sha256(image_bytes[:1024]).hexdigest()[:12]
